...
export default new Vuex.Store({
  ...
  actions: {
    FETCH_BOARD: async ({ commit }) => {
      let { data } = await Axios.get(process.env.VUE_APP_EBOARD_LINK);
      commit("SET_BOARD", data);
    },
    FETCH_POSTS: async ({ commit }) => {
      let { data } = await Axios.get(process.env.VUE_APP_POSTS_LINK);
      commit("SET_POSTS", data);
    },
    FETCH_EVENTS: async ({ commit }) => {
      let { data } = await Axios.get(process.env.VUE_APP_EVENTS_LINK);
