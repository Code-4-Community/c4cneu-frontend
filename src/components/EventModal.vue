<template>
  <div class="popup">
    <div class="popup-content" id="popup-content">
      ...
      <h2>{{ title }}</h2>
      <!-- typically you don't want to use breaks if it is only for spacing,
        should just add padding to the h2 -->
      <br />
      <p>{{ desc }}</p>
      <div v-if="past">
...
export default {
  ...
  props: {
    event: {
      // why would you validate that the prop is null?
  ...
  computed: {
    ...
    //codeIsValid: returns true if the code is valid.
    //Needs testing
    codeIsValid() {
      //TODO: Use regex to prevent leading zeros?
      // is 0001 not a valid code
